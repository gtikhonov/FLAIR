...
#load('.RData') # include the data used 
...
set.seed(123)
ptm <- proc.time()
model.gmf.newton.insects <- gmf(Y = Y_train_th15[,], X =X[,-1], p = k.insects,
                                ...
                                tol = 0.001, 
                                init='svd'); 
time.gmf.newton <- proc.time() - ptm; time.gmf.newton[3]
...
set.seed(123)
ptm <- proc.time()
model.gmf.airwls.insects <- gmf(Y = Y_train_th15[,], X =X[,-1], p = k.insects,
                                gamma=0.2, maxIter = 1000,
                                family = binomial(),
                                method = "airwls",
                                penaltyV = 0.2,
...
th <- 3
### - Data Splitting ####
train_test_val_split_th3 <- train_test_val_split(Y, th=15)
